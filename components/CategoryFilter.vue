<template>
  <div class="bg-violet-50">
    ...
    <TransitionRoot as="template" :show="open">
      <Dialog as="div" class="fixed inset-0 flex z-40 sm:hidden" @close="open = false">
        ...
        <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="translate-x-full">
          <div class="mr-auto relative max-w-xs w-full h-full bg-white shadow-xl py-4 pb-12 flex flex-col overflow-y-auto">
            <div class="px-4 flex items-center justify-between">
              <h2 class="text-lg font-medium text-gray-900">Filters</h2>
              <button type="button" class="mx-2 w-10 h-10 bg-white p-2 rounded-md flex items-center justify-center text-gray-400" @click="open = false">
    ...
    <div class="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
      <h1 class="text-3xl font-extrabold tracking-tight text-gray-900">محصولات کسا</h1>
...
<!--      <div class="relative z-10 bg-violet-50 border-b border-gray-400 pb-4">-->
...
<!--                        <div v-for="(option, optionIdx) in section.options" :key="option.value" class="flex items-center">-->
<!--                          <input :id="`filter-${section.id}-${optionIdx}`" :name="`${section.id}[]`" :value="option.value" type="checkbox" :checked="option.checked" class="h-4 w-4 border-gray-400 rounded text-indigo-600 focus:ring-indigo-500" />-->
...
<!--      &lt;!&ndash; Active filters &ndash;&gt;-->
<!--      <div class="bg-gray-100">-->
<!--        <div class="max-w-7xl mx-auto py-3 px-4 sm:flex sm:items-center sm:px-6 lg:px-8">-->
<!--          <h3 class="text-xs font-semibold uppercase tracking-wide text-gray-500  m-2">-->
<!--            Filters-->
...
const activeFilters = [{ value: 'فیلتر', label: 'فیلتر' }]
