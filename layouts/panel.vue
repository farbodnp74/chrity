    ...
    <Disclosure as="div" class="relative bg-sky-700 pb-32 overflow-hidden" v-slot="{ open }">
      ...
      <header class="relative py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 class="text-3xl font-extrabold text-white">پنل کاربری</h1>
        </div>
      </header>
    </Disclosure>
            ...
            <aside class="py-6 lg:col-span-3">
              <nav class="space-y-1">
                <a v-for="item in subNavigation" :key="item.name" :href="item.href"
                   :class="[item.address === this.name ? 'bg-teal-50 border-teal-500 text-teal-700 hover:bg-teal-50 hover:text-teal-700' : 'border-transparent text-gray-900 hover:bg-gray-50 hover:text-gray-900', 'group border-l-4 px-3 py-2 flex items-center text-sm font-medium']" :aria-current="item.current ? 'page' : undefined">
                  <component :is="item.icon" :class="[item.current ? 'text-teal-500 group-hover:text-teal-500' : 'text-gray-400 group-hover:text-gray-500', 'flex-shrink-0 m-1 mr-3 h-6 w-6']" aria-hidden="true" />
                  <span class="truncate">
                    {{ item.name }}
                  </span>
                </a>
...
import {
  ...
  CogIcon,
  CreditCardIcon, FolderIcon, HomeIcon,
  KeyIcon,
  MenuIcon,
...
const subNavigation = [
  { name: 'داشبورد', href: '/admin', icon: HomeIcon, current: false,address:'admin' },
  { name: 'دسته بندی', href: '/category/create', icon: UsersIcon, current: false ,address:'category-create'  },
  { name: 'محصولات', href: '/product/create', icon: UsersIcon, current: false ,  address:'product-create'},
  { name: 'خرید ها', href: '/check/create', icon: FolderIcon, current: false,address:'check-create' },
  { name: 'خروج', href: '/exit', icon: CalendarIcon, current: false,address:'exit' },
